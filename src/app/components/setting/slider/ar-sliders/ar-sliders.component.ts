...
@Component({
  selector: 'app-ar-sliders',
  templateUrl: './ar-sliders.component.html',
  styleUrls: ['./ar-sliders.component.scss']
})
export class ArSlidersComponent implements OnInit {
  public allLang = true;
  lang = "";
  ...
  getPosts() {
        ...
        this.dataService.getSlidersAR.subscribe((data) => {
          this.packages = data;
  ...
  onDeleteConfirm(slider) {
    ...
    //  category.confirm.resolve(category.newData);
      ...
      this.dataService.deleteSliderAR(slider.id);
    } else {
  ...
  onEdit(id){
    this.router.navigate(['/settings/add-slider/'],{ queryParams: {id: id,lang:'AR'}});
    }
